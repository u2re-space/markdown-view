...
var a = (() => {
...
}, se = /^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/, ce = /^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/, S = /^( {2,}|\\)\n(?!\s*$)/, le = /^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/, C = /[\p{P}\p{S}]/u, w = /[\s\p{P}\p{S}]/u, T = /[^\s\p{P}\p{S}]/u, ue = i(/^((?![*_])punctSpace)/, "u").replace(/punctSpace/g, w).getRegex(), E = /(?!~)[\p{P}\p{S}]/u, de = /(?!~)[\s\p{P}\p{S}]/u, fe = /(?:[^\s\p{P}\p{S}]|~)/u, D = /(?![*_])[\p{P}\p{S}]/u, pe = /(?![*_])[\s\p{P}\p{S}]/u, me = /(?:[^\s\p{P}\p{S}]|[*_])/u, he = i(/link|precode-code|html/, "g").replace("link", /\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-", a ? "(?<!`)()" : "(^^|[^`])").replace("code", /(?<b>`+)[^`]+\k<b>(?!`)/).replace("html", /<(?! )[^<>]*?>/).getRegex(), O = /^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/, ge = i(O, "u").replace(/punct/g, C).getRegex(), _e = i(O, "u").replace(/punct/g, E).getRegex(), k = "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)", ve = i(k, "gu").replace(/notPunctSpace/g, T).replace(/punctSpace/g, w).replace(/punct/g, C).getRegex(), ye = i(k, "gu").replace(/notPunctSpace/g, fe).replace(/punctSpace/g, de).replace(/punct/g, E).getRegex(), be = i("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)", "gu").replace(/notPunctSpace/g, T).replace(/punctSpace/g, w).replace(/punct/g, C).getRegex(), xe = i(/^~~?(?:((?!~)punct)|[^\s~])/, "u").replace(/punct/g, D).getRegex(), Se = i("^[^~]+(?=[^~])|(?!~)punct(~~?)(?=[\\s]|$)|notPunctSpace(~~?)(?!~)(?=punctSpace|$)|(?!~)punctSpace(~~?)(?=notPunctSpace)|[\\s](~~?)(?!~)(?=punct)|(?!~)punct(~~?)(?!~)(?=punct)|notPunctSpace(~~?)(?=notPunctSpace)", "gu").replace(/notPunctSpace/g, me).replace(/punctSpace/g, pe).replace(/punct/g, D).getRegex(), Ce = i(/\\(punct)/, "gu").replace(/punct/g, C).getRegex(), we = i(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme", /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email", /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(), Te = i(v).replace("(?:-->|$)", "-->").getRegex(), Ee = i("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment", Te).replace("attribute", /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(), A = /(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+[^`]*?`+(?!`)|[^\[\]\\`])*?/, De = i(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label", A).replace("href", /<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title", /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(), j = i(/^!?\[(label)\]\[(ref)\]/).replace("label", A).replace("ref", g).getRegex(), M = i(/^!?\[(ref)\](?:\[\])?/).replace("ref", g).getRegex(), Oe = i("reflink|nolink(?!\\()", "g").replace("reflink", j).replace("nolink", M).getRegex(), N = /[hH][tT][tT][pP][sS]?|[fF][tT][pP]/, P = {
	_backpedal: r,
	anyPunctuation: Ce,
	autolink: we,
	blockSkip: he,
	...
	...P,
	link: i(/^!?\[(label)\]\((.*?)\)/).replace("label", A).getRegex(),
	reflink: i(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", A).getRegex()
}, F = {
	...P,
	...
	delLDelim: xe,
	delRDelim: Se,
	url: i(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol", N).replace("email", /[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),
		...
		return e.forEach((e) => {
			...
			if (e.hooks) {
				...
				for (let n in e.hooks) {
					...
					Y.passThroughHooks.has(n) ? t[r] = (e) => {
...
}, Q.getDefaults = e, Q.defaults = t, Q.use = function(...e) {
...
var Fe = Q.options, Ie = Q.setOptions, Le = Q.use, $ = Q.walkTokens, Re = Q.parseInline, ze = Q, Be = J.parse, Ve = G.lex;
//#endregion
export { Y as Hooks, G as Lexer, X as Marked, J as Parser, K as Renderer, q as TextRenderer, W as Tokenizer, t as defaults, e as getDefaults, Ve as lexer, Q as marked, Fe as options, ze as parse, Re as parseInline, Be as parser, Ie as setOptions, Le as use, $ as walkTokens };
