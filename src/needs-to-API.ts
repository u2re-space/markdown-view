...
import { dataUrlToFile, isAndroidLocalShareUri } from "com/routing/channel/sku-ingress";
import {
    classifyOpenKind,
...
export const CwViewViewer = createViewConstructor(TAG, (Base: any)=>{
    ...
    private visibilityOpenListener: (() => void) | null = null;
    private capacitorOpenPull: Promise<void> = Promise.resolve();
    ...
    constructor(options: ViewerOptions = {}) {
        ...
        /* WHY: applyRouteParams (share / SKU handoff) must win over the last saved document. */
    ...
    private setupEventHandlers(rawElement?: HTMLPreElement): void {
        ...
        if (dropZone) {
            ...
            /* WHY: window capture drop is gated; the highlighted zone must still ingest. */
    ...
    private handleOpen(): void {
        void (async () => {
            ...
            const ok = await this.ingestOpenedFile(picked.file, {
    ...
    private handleOpenInputFallback(): void {
        ...
        input.onchange = async () => {
            ...
            if (file) {
                try {
                    const ok = await this.ingestOpenedFile(file, { filename: file.name });
                    if (ok) this.showMessage(`Opened ${file.name}`);
                } catch (error) {
    ...
    private viewerAcceptsGlobalInput(): boolean {
        ...
        /* WHY: Document SKU can paint before onShow; a connected shell still owns drop. */
    ...
    private async ingestDroppedFiles(dt: DataTransfer | null | undefined): Promise<void> {
        ...
        for (const item of items) {
            ...
            /* INVARIANT: Chromium empties `dt.files` after getAsFileSystemHandle — read File first. */
            ...
            try {
                ...
                if (handle?.kind === "file" && "getFile" in handle) {
                    const fromHandle = await (handle as FileSystemFileHandle).getFile();
        ...
        if (directoryHandle) {
            ...
            const ok = await this.ingestOpenedFile(pick, { virtualPath: `${root}${rel}`, filename: pick.name });
            if (ok) this.showMessage(`Opened ${pick.name}`);
            return;
        }
        ...
        if (files.length > 0) {
            ...
            try {
                const ok = await this.ingestOpenedFile(pick, { filename: pick.name });
                if (ok) this.showMessage(`Loaded ${pick.name}`);
            } catch {
    ...
    private async ingestPastedPayload(files: File[], textPlain: string | undefined): Promise<void> {
        if (files.length > 0) {
            ...
            try {
                const ok = await this.ingestOpenedFile(pick, { filename: pick.name });
                if (ok) this.showMessage(`Opened ${pick.name || "pasted document"}`);
                return;
            } catch (error) {
    ...
    private async paintSharedIngressFile(
        file: File,
        filename?: string,
        source?: string | null
    ): Promise<boolean> {
     ...
     */
    private async ingestOpenedFile(
        file: File,
        opts?: { virtualPath?: string | null; filename?: string; channel?: OpenChannel | OpenChannel[] }
    ): Promise<boolean> {
        /* INVARIANT: Document SKU drop/paste paints here. Settings workcenter/explorer sinks have no host. */
        if (inferCwspSkuFromLocation() === "document") {
            if (this.looksLikeBinaryPreviewFile(file) || this.isTextLikeFile(file)) {
                return this.paintSharedIngressFile(file, opts?.filename, opts?.virtualPath ?? null);
            }
        ...
        if (sink === "workcenter") {
            return this.handoffOpenedFile("workcenter", file, opts?.virtualPath);
        }
        if (sink === "explorer") {
            return this.handoffOpenedFile("explorer", file, opts?.virtualPath);
        }
        if (sink === "external" || sink === "system") {
            ...
            try {
                ...
                /* fall through to in-viewer preview */
            }
        }
        if (this.looksLikeBinaryPreviewFile(file)) {
            this.showSharedBinaryPreview(file);
    ...
    private async handoffOpenedFile(
        dest: "workcenter" | "explorer",
        file: File,
        ...
        if (shouldHandoffViewToSibling(dest)) {
            try {
                ...
                stashSkuHandoff({ dest, content, filename, src });
            } catch {
                stashSkuHandoff({ dest, filename, src });
            }
        ...
        try {
            const sent = await sendViewProtocolMessage({
                ...
                attachments: [{ data: file, source: "viewer-open-policy" }],
                data: { filename, path: src, source: src }
            });
            if (sent) {
    ...
    private onMount(): void {
                ...
                const file =
                    detail?.file instanceof File
                        ...
                        : Array.isArray(detail?.files)
                          ? detail.files.find((row): row is File => row instanceof File)
                          : undefined;
                /* INVARIANT: PWA warm share paints in-place. preventDefault tells the page the viewer accepted it. */
                if (file) {
                    ce.preventDefault();
                    void this.paintSharedIngressFile(file, detail?.filename, detail?.src || null);
                    return;
                }
        ...
        if (!this.visibilityOpenListener) {
            this.visibilityOpenListener = (): void => {
                if (document.visibilityState === "visible") void this.pullCapacitorPendingOpen();
            };
     ...
     */
    private pullCapacitorPendingOpen(): Promise<void> {
        this.capacitorOpenPull = this.capacitorOpenPull.then(
            () => this.pullCapacitorPendingOpenOnce(),
            () => this.pullCapacitorPendingOpenOnce()
        );
        return this.capacitorOpenPull;
    }

    private async pullCapacitorPendingOpenOnce(): Promise<void> {
        try {
            ...
            const flagged =
                echo.hasFile === true || echo.hasFile === "true" || echo.hasFile === 1 || echo.hasFile === "1";
            ...
            const wantFile =
                ...
                /\.(md|markdown|txt|pdf|png|jpe?g|gif|webp|html?)$/i.test(nameHint);
            const pullFile = async (): Promise<File | null> => {
            ...
            let file: File | null = wantFile ? await pullFile() : null;
            if (wantFile && !file) {
            ...
            let applied = false;
            if (file) {
                /* WHY: native Open-with/Share on this SKU must paint here — open-policy handoff leaves a blank viewer. */
                ...
                applied = await this.openMarkdownFromUrl(url, filename);
            }
            if (!applied) return;
    ...
    private onShow(): void {
        ...
        const handed = takeSkuHandoff("viewer", "document");
        if (handed?.content?.trim()) {
            this.ingestOpenedMarkdownBody(handed.content, handed.filename, handed.src);
        }
        void this.pullCapacitorPendingOpen();
    ...
    async handleMessage(message: unknown): Promise<void> {
        ...
        if (fileEarly && this.looksLikeBinaryPreviewFile(fileEarly)) {
            const channel: OpenChannel[] = fromShareTarget
                ? ["share-target"]
                : fromLaunchQueue
                  ? ["launch-queue"]
                  : ["open"];
            await this.ingestOpenedFile(fileEarly, {
                virtualPath: this.pickDocumentSourcePath(
                    payload?.virtualPath,
                    payload?.src,
                    payload?.path,
                    payload?.hint?.source,
